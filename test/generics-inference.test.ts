...
describe('record and list hovers', () => {
  it('creates hover with inferred List<Record> type', () => {
    ...
    expect(mapHover?.markdown).toContain('Iterable<T> map<T>(T Function(({String title, String content})) toElement)')
    
    // Check that the callback parameter 'detail' has the inferred record type
    const detailHover = hovers.find(h => h.expectedValue === 'detail')
    expect(detailHover).toBeDefined()
    expect(detailHover?.markdown).toContain('({String title, String content})')
